...
const guardarUsuario = async (e) => {
    ...
    try {
        ...
        const url = "/proyecto011/registro/guardar";
        const config = {
            method: 'POST',
            body
        };
...
const buscarUsuarios = async () => {
    try {
        ...
        const respuesta = await fetch('/proyecto011/registro/buscarAPI');
        const data = await respuesta.json();
        ...
        columns: [
            ...
            },
            {
                ...
                render: (data, type, row) => {
                    if (data && data.trim() !== '') {
                        return `<img src="/proyecto011/storage/fotosUsuarios/${data}" 
                                    alt="Foto" class="img-thumbnail rounded-circle foto-usuario" 
                                    style="width: 40px; height: 40px; object-fit: cover; cursor: pointer;"
                                    data-imagen="/proyecto011/storage/fotosUsuarios/${data}"
...
const modificarUsuario = async (e) => {
    ...
    try {
        const body = new FormData(formUsuario);
        const respuesta = await fetch('/proyecto011/registro/modificarAPI', {
            method: 'POST',
            body
        });
...
const eliminarUsuario = async (e) => {
    ...
    if (confirmacion.isConfirmed) {
        try {
            const respuesta = await fetch(`/proyecto011/registro/eliminarAPI?id=${id}`);
            const data = await respuesta.json();
